...
<title>USNow — The Question is the Answer.</title>
<style>
...
.layer-main-label{font-weight:700;letter-spacing:0.3em;font-family:'Courier New',monospace;white-space:nowrap;transition:text-shadow 0.2s;}
.layer-sub-label{display:none;}
.layer-band:hover .layer-main-label{color:#ffffff!important;text-shadow:0 0 28px rgba(255,255,255,0.55),0 0 60px rgba(255,255,255,0.18),0 2px 8px rgba(0,0,0,0.9)!important;letter-spacing:0.38em!important;}
...
#modal-overlay{display:none;position:fixed;inset:0;z-index:3000;background:rgba(2,4,10,0.84);backdrop-filter:none;-webkit-backdrop-filter:none;align-items:flex-end;justify-content:center;padding:0 10px 16px;}
#modal-overlay.open{display:flex;}
#modal-overlay.zoomed-safari{align-items:center;padding:12px;background:rgba(2,4,10,0.92);backdrop-filter:none;-webkit-backdrop-filter:none;}
#modal-overlay.zoomed-safari #modal-glow-halo{display:none!important;opacity:0!important;}
#modal-overlay.zoomed-safari #modal-box{max-height:78vh;border-radius:20px;}
#modal-glow-halo{display:none !important;position:fixed;bottom:-60px;left:50%;transform:translateX(-50%);width:720px;height:420px;border-radius:50%;pointer-events:none;z-index:2999;transition:none;opacity:0;filter:none;}
#modal-overlay.open #modal-glow-halo{opacity:0;}
#modal-box{background:radial-gradient(ellipse 120% 60% at 50% 0%,rgba(255,255,255,0.09) 0%,rgba(255,255,255,0.02) 35%,transparent 65%),linear-gradient(160deg,#131e35 0%,#0c1228 100%);border:3px solid rgba(255,255,255,0.1);border-radius:28px 28px 18px 18px;padding:40px 26px 28px;max-width:560px;width:100%;position:relative;max-height:88vh;overflow-y:auto;-webkit-overflow-scrolling:touch;transition:box-shadow 0.35s ease,border-color 0.35s ease,transform 0.25s ease;}
#modal-box::before{content:'';position:absolute;inset:0;border-radius:28px 28px 18px 18px;pointer-events:none;z-index:1;box-shadow:inset 0 0 0 1px rgba(255,255,255,0.05),inset 0 1px 0 rgba(255,255,255,0.14);}
...
.modal-layer-tag{font-size:10px;letter-spacing:0.3em;font-family:'Courier New',monospace;opacity:1;margin-bottom:8px;}
...
.modal-links-label{font-size:10px;letter-spacing:0.25em;font-family:'Courier New',monospace;margin-bottom:12px;opacity:1;}
.modal-links-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1px;background:rgba(58,74,90,0.15);border-radius:8px;overflow:hidden;margin-top:4px;}.modal-link{display:flex;align-items:center;gap:10px;padding:13px 14px;background:#060b18;text-decoration:none;transition:background 0.2s;line-height:1.3;}@media(max-width:480px){.modal-links-grid{grid-template-columns:1fr;}}
...
.modal-bottompills-label{font-size:10px;letter-spacing:0.25em;font-family:'Courier New',monospace;margin-bottom:12px;opacity:1;}
.modal-orb-tag{font-size:11px;letter-spacing:0.04em;font-family:'Courier New',monospace;font-weight:500;border-radius:20px;padding:6px 12px 6px 8px;display:inline-flex;align-items:center;gap:7px;margin:0 4px 6px 0;cursor:pointer;transition:all 0.2s;text-decoration:none;box-shadow:inset 0 1px 0 rgba(255,255,255,0.06),0 1px 3px rgba(0,0,0,0.25);}
...
.modal-orb-dot{width:16px;height:16px;border-radius:50%;flex-shrink:0;display:inline-block;position:relative;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.45),0 0 4px rgba(0,0,0,0.3);}
...
@media(max-width:520px){.nav-links a:not(.nav-cta){display:none;}.layer-main-label{letter-spacing:0.15em!important;}.nav-fields{display:none!important;}}
</style>
<script async src="https://www.googletagmanager.com/gtag/js?id=G-DPZHKNB3E9"></script>
<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','G-DPZHKNB3E9');</script>
</head>
<body>

<div id="ticker"><div id="ticker-inner">
<span>◎ Non-partisan · Non-denominational · Non-negotiable.</span>
<span>◎ The question is the answer. The data is the argument.</span>
<span>◎ Some guy with AI — minding propaganda &amp; media mind control.</span>
<span>◎ Share it and Help Build it.</span>
<span>◎ The US in USNow is not one thing.</span>
<span>◎ Helping AI expose corruption, abuse, injustice, and overreach — wherever it exists.</span>
<span>◎ See the Indexed and Interactive World Compendium.</span>
<span>◎ Non-partisan · Non-denominational · Non-negotiable.</span>
<span>◎ The question is the answer. The data is the argument.</span>
<span>◎ Some guy with AI — minding propaganda &amp; media mind control.</span>
<span>◎ Share it and Help Build it.</span>
...
<nav>
  <div class="nav-logo" style="margin-bottom:8px;flex-direction:column;align-items:flex-start;gap:2px;">
    <div style="display:flex;align-items:center;gap:8px;">
      <span class="us">US</span>
      <span class="now">NOW</span>
    </div>
    <span style="font-size:7px;letter-spacing:0.13em;color:rgba(201,168,76,0.48);font-family:'Courier New',monospace;white-space:nowrap;">THE QUESTION IS THE ANSWER.</span>
  </div>

  <!-- Sign + timestamp — flex center between logo and button -->
  <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;margin:0 18px;margin-bottom:0;">
    <span id="nav-sign" style="display:inline-flex;align-items:center;justify-content:center;min-width:160px;max-width:420px;padding:0 14px;height:20px;border:1px solid rgba(201,168,76,0.38);border-radius:3px;background:rgba(201,168,76,0.04);overflow:visible;pointer-events:none;transition:opacity 0.25s;position:relative;margin-bottom:1px;animation:navSignPulse 4s ease-in-out infinite;white-space:nowrap;">
      <!-- Ghost sizer — invisible, drives container width to longest string -->
      <span style="visibility:hidden;font-size:10px;letter-spacing:0.22em;font-family:'Courier New',monospace;white-space:nowrap;pointer-events:none;">CLARITY IN PROGRESS</span>
      <span style="position:absolute;text-align:center;font-size:10px;letter-spacing:0.22em;color:#c9a84c;font-family:'Courier New',monospace;white-space:nowrap;animation:navFrame1 4s ease-in-out infinite;">CLARITY IN PROGRESS</span>
      <span style="position:absolute;text-align:center;font-size:10px;letter-spacing:0.22em;color:#c9a84c;font-family:'Courier New',monospace;white-space:nowrap;animation:navFrame2 4s ease-in-out infinite;">ADD YOUR VOICE</span>
    </span>
    <span id="hero-date" style="font-size:7px;letter-spacing:0.14em;color:rgba(201,168,76,0.28);font-family:'Courier New',monospace;white-space:nowrap;"></span>
  </div>
  <!-- Contact toggle -->
  <div style="display:flex;align-items:center;gap:8px;overflow:hidden;margin-bottom:8px;">
    <!-- Button -->
    <button id="nav-contact-btn" onclick="openNavForm()"
      style="font-size:11px;letter-spacing:0.04em;background:transparent;border:1px solid rgba(122,96,48,0.45);padding:5px 14px;border-radius:16px;cursor:pointer;font-family:'Courier New',monospace;white-space:nowrap;transition:all 0.25s;"
      onmouseover="this.style.borderColor='#c9a84c'"
      onmouseout="this.style.borderColor='rgba(122,96,48,0.45)'">
      <span style="color:#8aa0b8;letter-spacing:0.06em;">Contact </span><span style="color:#c9a84c;font-weight:700;letter-spacing:0.18em;">US</span><span style="color:#8aa0b8;letter-spacing:0.1em;">Now</span>
    </button>
    <!-- Fields (hidden by default) -->
    <div id="nav-form-wrap" style="display:none;align-items:center;gap:7px;animation:fadeIn 0.25s ease;">
      <input id="nav-email" type="email" placeholder="email (optional)"
        style="background:rgba(255,255,255,0.05);border:none;border-bottom:1px solid rgba(122,96,48,0.3);padding:4px 7px;font-size:10px;color:#8aa0b8;font-family:'Courier New',monospace;outline:none;width:122px;border-radius:3px 3px 0 0;"
        onfocus="this.style.borderBottomColor='rgba(201,168,76,0.6)'"
        onblur="this.style.borderBottomColor='rgba(122,96,48,0.3)'"/>
      <input id="nav-note" type="text" placeholder="your thought..."
        style="background:rgba(255,255,255,0.05);border:none;border-bottom:1px solid rgba(122,96,48,0.3);padding:4px 7px;font-size:10px;color:#8aa0b8;font-family:'Courier New',monospace;outline:none;width:160px;border-radius:3px 3px 0 0;"
        onfocus="this.style.borderBottomColor='rgba(201,168,76,0.6)'"
        onblur="this.style.borderBottomColor='rgba(122,96,48,0.3)'"/>
      <button onclick="submitNavThought()"
        style="font-size:8px;letter-spacing:0.22em;color:#c9a84c;background:transparent;border:1px solid rgba(122,96,48,0.4);padding:4px 11px;border-radius:12px;cursor:pointer;font-family:'Courier New',monospace;white-space:nowrap;"
        onmouseover="this.style.borderColor='#c9a84c'"
        onmouseout="this.style.borderColor='rgba(122,96,48,0.4)'">SEND →</button>
    </div>
  </div>

</nav>

<!-- Nav submit confirmation — floats above everything -->
<div id="nav-confirm" style="display:none;position:fixed;top:84px;left:50%;transform:translateX(-50%);z-index:9999;background:linear-gradient(160deg,#0d1a2a,#071018);border:1px solid rgba(74,154,112,0.55);border-radius:10px;padding:14px 24px;text-align:center;box-shadow:0 0 40px rgba(74,154,112,0.18),0 8px 32px rgba(0,0,0,0.7);animation:fadeIn 0.3s ease;pointer-events:none;">
  <div style="font-size:11px;letter-spacing:0.18em;color:#4a9a70;font-family:'Courier New',monospace;margin-bottom:5px;">✓ RECEIVED</div>
  <div style="font-size:11px;color:#8ab8a0;font-family:'Georgia',serif;line-height:1.6;">Thanks for your input.<br>If you left an email, you'll hear back.</div>
</div>
<section id="scale" style="margin-top:74px;">
  <div id="stars"></div>
  <svg id="scale-svg" viewBox="0 0 1000 700" preserveAspectRatio="none"></svg>
  <div id="layer-bands"></div>
  <div id="core-wrap">
    <div id="core-orbs"></div>
    <div id="core-btn" onclick="openLayer(0)"><div>ME,<br>MYSELF<br>&amp; I</div></div>
    <!-- Foundation half-moons injected by JS -->
  </div>

</section>

<div id="modal-overlay" onclick="cardNavClose()">
  <div id="modal-glow-halo"></div>
  <div id="modal-box" onclick="event.stopPropagation()">
    <div class="modal-drag" id="modal-drag"></div>
    <div id="card-nav-bar" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
      <div id="modal-layer-tag" style="font-size:10px;letter-spacing:0.3em;font-family:'Courier New',monospace;opacity:1;line-height:1.6;"></div>
      <button id="modal-close" onclick="cardNavBack()" style="font-size:10px;letter-spacing:0.18em;font-family:'Courier New',monospace;background:transparent;border:1px solid transparent;padding:6px 14px;border-radius:14px;cursor:pointer;transition:all 0.2s;"></button>
    </div>
    <h2 id="modal-title"></h2>
    <div id="modal-sub" style="position:static;opacity:1;transform:none;font-size:12px;font-family:'Courier New',monospace;letter-spacing:0.08em;margin-bottom:14px;display:none;"></div>
    <div id="modal-toppills-section" style="margin-bottom:18px;display:none;"><div id="modal-toppills-list" style="display:grid;grid-template-columns:repeat(2,1fr);gap:5px;"></div></div>
    <p id="modal-desc"></p>
    <div id="modal-bottompills-section" style="margin-top:20px;border-top:1px solid rgba(128,128,128,0.1);padding-top:16px;display:none;"><div class="modal-bottompills-label" id="modal-bottompills-label" style="font-size:10px;letter-spacing:0.25em;font-family:'Courier New',monospace;margin-bottom:12px;opacity:0.7;">ON-SITE CONNECTIONS</div><div id="modal-bottompills-list" style="display:grid;grid-template-columns:repeat(2,1fr);gap:5px;"></div></div>
    <div id="modal-links" style="margin-top:18px;border-top:1px solid rgba(128,128,128,0.1);padding-top:16px;display:none;"><div class="modal-links-label" id="modal-links-label">OFF-SITE CONNECTIONS</div><div id="modal-links-list" class="modal-links-grid"></div></div>
  </div>
</div>

<section id="hero">
  <div class="inner">
    <div class="hero-tap-hint">↑ tap any layer or orb above to explore</div>
    <div class="hero-eyebrow">WHAT IS US · NOW</div>
    <h1 class="hero-h1">The <em>US</em> in USNow<br>is not one thing.</h1>
    <div class="hero-items">
      <div class="hero-item"><div class="hero-rule"></div><p><strong>The US  in our country</strong> — in the U.S. it's WE the People, our history, our government, the American dream - and the reality - along with our mutual obligations with the rest of the world.</p></div>
      <div class="hero-item"><div class="hero-rule"></div><p><strong>The US  in our groups</strong> — our state, our party, religion or sport team - the groups we are in or those we join - how they can bring us together and how they divide and separate us as well.</p></div>
      <div class="hero-item"><div class="hero-rule"></div><p><strong>The US  in our relationships</strong> — the most natural bond - our family, our parents, our children and siblings - and those we choose or unchoose to be closest to us - spouse, friends, partners.</p></div>
      <div class="hero-item"><div class="hero-rule"></div><p><strong>The US  within ourselves</strong> — the innermost us. The me and the I. Our inner voice and the one who heeds it. The us that ultimately determienes what we say and do and why.</p></div>
    </div>
    <div class="hero-btns">
      <a href="#scale" class="btn-gold">EXPLORE THE SCALE ↑</a>
      <a href="#data" class="btn-outline">SEE THE DATA →</a>
    </div>
  </div>
</section>

<section id="thoughts" style="position:relative;padding:52px 24px 56px;overflow:hidden;border-top:1px solid rgba(200,120,80,0.18);border-bottom:1px solid rgba(200,120,80,0.12);">

  <!-- Warm glow background -->
  <div style="position:absolute;inset:0;background:#06080f;z-index:0;"></div>
  <div style="position:absolute;inset:0;background:radial-gradient(ellipse 80% 100% at 50% 110%,rgba(200,100,40,0.22) 0%,rgba(180,80,20,0.10) 45%,transparent 72%);z-index:1;pointer-events:none;"></div>
  <div style="position:absolute;bottom:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,rgba(200,100,40,0.35),transparent);z-index:2;"></div>

  <div style="position:relative;z-index:3;max-width:560px;margin:0 auto;text-align:center;">

    <!-- Branded header -->
    <div style="margin-bottom:22px;">
      <div style="display:inline-flex;align-items:baseline;gap:6px;margin-bottom:10px;">
        <span style="font-size:11px;letter-spacing:0.12em;color:#8aa0b8;font-family:'Courier New',monospace;">Contact</span>
        <span style="font-size:22px;font-weight:700;letter-spacing:0.22em;color:#c9a84c;font-family:'Courier New',monospace;text-shadow:0 0 24px rgba(201,168,76,0.45),0 0 60px rgba(201,168,76,0.18);">US</span><span style="font-size:22px;letter-spacing:0.14em;color:#8aa0b8;font-family:'Courier New',monospace;">Now</span>
      </div>
      <div style="width:60px;height:1px;background:linear-gradient(90deg,transparent,rgba(201,168,76,0.5),transparent);margin:0 auto 14px;"></div>
      <p style="font-size:13px;font-weight:300;color:#7a8fa8;font-family:'Georgia',serif;line-height:1.7;margin:0;">Leave feedback - Your thoughts, suggestions, and comments are welcome.<br>Name and Email Optional. No Strings. Help Build UsNow.</p>
    </div>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;align-items:flex-end;">
      <input id="t-email" type="email" placeholder="email (optional)"
        style="background:rgba(255,255,255,0.06);border:none;border-bottom:1px solid rgba(200,120,60,0.35);padding:8px 10px;font-size:12px;color:#a0b4c8;font-family:'Courier New',monospace;outline:none;width:165px;border-radius:4px 4px 0 0;transition:border-color 0.2s;"
        onfocus="this.style.borderBottomColor='rgba(201,168,76,0.7)'"
        onblur="this.style.borderBottomColor='rgba(200,120,60,0.35)'"/>
      <input id="t-note" type="text" placeholder="your thought..."
        style="background:rgba(255,255,255,0.06);border:none;border-bottom:1px solid rgba(200,120,60,0.35);padding:8px 10px;font-size:12px;color:#a0b4c8;font-family:'Courier New',monospace;outline:none;width:240px;border-radius:4px 4px 0 0;transition:border-color 0.2s;"
        onfocus="this.style.borderBottomColor='rgba(201,168,76,0.7)'"
        onblur="this.style.borderBottomColor='rgba(200,120,60,0.35)'"/>
      <button onclick="submitThought()"
        style="font-size:9px;letter-spacing:0.22em;color:#c9a84c;background:transparent;border:1px solid rgba(200,120,60,0.5);padding:7px 18px;border-radius:16px;cursor:pointer;font-family:'Courier New',monospace;transition:all 0.2s;"
        onmouseover="this.style.borderColor='#c9a84c';this.style.boxShadow='0 0 12px rgba(201,168,76,0.2)'"
        onmouseout="this.style.borderColor='rgba(200,120,60,0.5)';this.style.boxShadow='none'">SEND →</button>
    </div>
    <div style="margin-top:14px;min-height:18px;">
      <span id="t-confirm" style="font-size:10px;color:#4a9a70;font-family:'Courier New',monospace;letter-spacing:0.08em;opacity:0;transition:opacity 0.4s;">✓ Thanks for your input. If you left an email, you'll hear back.</span>
    </div>

  </div>
</section>

<!-- WHAT IS USNOW — mission statement -->
<section id="about" style="background:#04080f;padding:64px 24px 72px;border-top:1px solid rgba(201,168,76,0.18);border-bottom:1px solid rgba(201,168,76,0.10);position:relative;overflow:hidden;">

  <div style="position:absolute;inset:0;background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(201,168,76,0.04) 0%,transparent 70%);pointer-events:none;"></div>

  <div style="font-family:'Barlow Condensed',sans-serif;font-size:10px;font-weight:600;letter-spacing:4px;color:rgba(201,168,76,0.55);text-transform:uppercase;text-align:center;margin-bottom:28px;">WHAT IS THIS</div>

  <div style="width:40px;height:1px;background:linear-gradient(90deg,transparent,rgba(201,168,76,0.6),transparent);margin:0 auto 24px;"></div>

  <h2 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(38px,8vw,62px);letter-spacing:3px;text-align:center;margin:0 0 48px;line-height:1;"><span style="color:#c9a84c;">Us</span><span style="color:#e8e0d0;">Now</span></h2>

  <div style="max-width:680px;margin:0 auto;position:relative;z-index:1;">

    <div style="position:absolute;left:-20px;top:6px;width:2px;height:calc(100% - 12px);background:linear-gradient(180deg,rgba(201,168,76,0.5),rgba(201,168,76,0.08));border-radius:2px;"></div>

    <p style="font-family:'Barlow Condensed',sans-serif;font-size:clamp(16px,2.4vw,19px);font-weight:400;line-height:1.75;color:rgba(232,224,208,0.82);margin:0 0 28px;">
      You are living inside more information than any human being in history has ever had to process. News, politics, history, science, war, money, culture — all of it arriving simultaneously, all of it demanding a reaction. Most of it is designed to make you <em>feel</em> something specific before you've had a chance to <em>think</em> about anything.
    </p>

    <p style="font-family:'Barlow Condensed',sans-serif;font-size:clamp(16px,2.4vw,19px);font-weight:400;line-height:1.75;color:rgba(232,224,208,0.82);margin:0 0 28px;">
      USNow is a place to stop and see where we actually are. Not just in the news cycle, but in the larger picture — the one that includes everything that happened before this moment and everything that has unfolded from it. And where you fit inside all of that.
    </p>

    <p style="font-family:'Barlow Condensed',sans-serif;font-size:clamp(16px,2.4vw,19px);font-weight:400;line-height:1.75;color:rgba(232,224,208,0.82);margin:0 0 28px;">
      We start with what's documented and work outward toward what it means.
    </p>

    <p style="font-family:'Barlow Condensed',sans-serif;font-size:clamp(16px,2.4vw,19px);font-weight:400;line-height:1.75;color:rgba(232,224,208,0.82);margin:0 0 28px;">
      The goal isn't to tell you what to think. It's to give you enough of the real picture that you can decide for yourself — not as a consumer of someone else's narrative, but as a citizen. As a neighbor. As a human, at your own pace, without all the noise.
    </p>

    <p style="font-family:'Barlow Condensed',sans-serif;font-size:clamp(16px,2.4vw,19px);font-weight:400;line-height:1.75;color:rgba(232,224,208,0.82);margin:0;">
      Leave a note, a comment, or a suggested topic. We're just getting started and everything is always changing.
    </p>

  </div>

  <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin-top:52px;">
    <div style="width:32px;height:1px;background:rgba(201,168,76,0.3);"></div>
    <div style="width:5px;height:5px;border:1px solid rgba(201,168,76,0.5);transform:rotate(45deg);"></div>
    <div style="width:32px;height:1px;background:rgba(201,168,76,0.3);"></div>
  </div>
...
<section id="sources">
  <div class="inner">
    <div class="sources-intro">
      <div class="section-eyebrow">ESSENTIAL SOURCES</div>
      <p>The information is there. These are the doors.</p>
    </div>
    <div class="sources-grid">
      <a href="https://turbovote.org" target="_blank" class="source-item"><span class="source-emoji">🗳️</span><div><div class="source-name">TurboVote</div><div class="source-tag">VOTE</div></div></a>
      <a href="https://www.opensecrets.org" target="_blank" class="source-item"><span class="source-emoji">💰</span><div><div class="source-name">OpenSecrets</div><div class="source-tag">MONEY</div></div></a>
      <a href="https://www.followthemoney.org" target="_blank" class="source-item"><span class="source-emoji">🧾</span><div><div class="source-name">FollowTheMoney</div><div class="source-tag">MONEY</div></div></a>
      <a href="https://www.propublica.org" target="_blank" class="source-item"><span class="source-emoji">📰</span><div><div class="source-name">ProPublica</div><div class="source-tag">JOURNALISM</div></div></a>
      <a href="https://www.trackaipac.com" target="_blank" class="source-item"><span class="source-emoji">🔍</span><div><div class="source-name">TrackAIPAC</div><div class="source-tag">LOBBYING</div></div></a>
      <a href="https://foreignassistance.gov" target="_blank" class="source-item"><span class="source-emoji">🌍</span><div><div class="source-name">ForeignAssistance.gov</div><div class="source-tag">AID</div></div></a>
      <a href="https://quincyinst.org" target="_blank" class="source-item"><span class="source-emoji">🕊️</span><div><div class="source-name">Quincy Institute</div><div class="source-tag">MILITARY</div></div></a>
      <a href="https://watson.brown.edu/costsofwar" target="_blank" class="source-item"><span class="source-emoji">⚔️</span><div><div class="source-name">Costs of War</div><div class="source-tag">MILITARY</div></div></a>
      <a href="https://www.pewresearch.org" target="_blank" class="source-item"><span class="source-emoji">📈</span><div><div class="source-name">Pew Research</div><div class="source-tag">DATA</div></div></a>
      <a href="https://www.congress.gov" target="_blank" class="source-item"><span class="source-emoji">📜</span><div><div class="source-name">Congress.gov</div><div class="source-tag">PRIMARY</div></div></a>
      <a href="https://www.cbo.gov" target="_blank" class="source-item"><span class="source-emoji">🔢</span><div><div class="source-name">CBO</div><div class="source-tag">BUDGET</div></div></a>
      <a href="https://www.govtrack.us" target="_blank" class="source-item"><span class="source-emoji">🏛️</span><div><div class="source-name">GovTrack</div><div class="source-tag">CONGRESS</div></div></a>
      <a href="https://ballotpedia.org" target="_blank" class="source-item"><span class="source-emoji">📋</span><div><div class="source-name">Ballotpedia</div><div class="source-tag">ELECTIONS</div></div></a>
      <a href="https://www.usaspending.gov" target="_blank" class="source-item"><span class="source-emoji">📊</span><div><div class="source-name">USASpending.gov</div><div class="source-tag">BUDGET</div></div></a>
    </div>
  </div>
</section>

<section style="background:#020508;padding:52px 24px;border-top:1px solid rgba(58,74,90,0.13);">
  <div style="max-width:840px;margin:0 auto;">
    <div style="text-align:center;margin-bottom:32px;">
      <div style="font-size:10px;letter-spacing:0.45em;color:#7a6030;font-family:'Courier New',monospace;margin-bottom:10px;">EVEN MORE ESSENTIAL SOURCES</div>
      <p style="font-size:13px;color:#6a7a8a;font-family:'Georgia',serif;font-style:italic;margin:0;line-height:1.7;">All free. All credible. All pointing inward and outward at once.</p>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(186px,1fr));gap:1px;background:rgba(58,74,90,0.15);">
      <a href="https://medlineplus.gov" target="_blank" class="source-item" style="border-left:2px solid #3a9e5f22;"><span class="source-emoji">🫀</span><div><div class="source-name">NIH MedlinePlus</div><div class="source-tag" style="color:#3a9e5f;">BODY</div></div></a>
      <a href="https://wonder.cdc.gov" target="_blank" class="source-item" style="border-left:2px solid #3a9e5f22;"><span class="source-emoji">🧬</span><div><div class="source-name">CDC WONDER</div><div class="source-tag" style="color:#3a9e5f;">BODY · DATA</div></div></a>
      <a href="https://www.themarginalian.org" target="_blank" class="source-item" style="border-left:2px solid #7a5fd422;"><span class="source-emoji">🔭</span><div><div class="source-name">The Marginalian</div><div class="source-tag" style="color:#7a5fd4;">MIND · IDEAS</div></div></a>
      <a href="https://aeon.co" target="_blank" class="source-item" style="border-left:2px solid #7a5fd422;"><span class="source-emoji">🧠</span><div><div class="source-name">Aeon</div><div class="source-tag" style="color:#7a5fd4;">MIND · PHILOSOPHY</div></div></a>
      <a href="https://nautil.us" target="_blank" class="source-item" style="border-left:2px solid #7a5fd422;"><span class="source-emoji">🌊</span><div><div class="source-name">Nautilus</div><div class="source-tag" style="color:#7a5fd4;">MIND · SCIENCE</div></div></a>
      <a href="https://www.khanacademy.org" target="_blank" class="source-item" style="border-left:2px solid #7a5fd422;"><span class="source-emoji">📐</span><div><div class="source-name">Khan Academy</div><div class="source-tag" style="color:#7a5fd4;">MIND · LEARN</div></div></a>
      <a href="https://onbeing.org" target="_blank" class="source-item" style="border-left:2px solid #d45f8a22;"><span class="source-emoji">🕊️</span><div><div class="source-name">On Being</div><div class="source-tag" style="color:#d45f8a;">SOUL · SPIRIT</div></div></a>
      <a href="https://insighttimer.com" target="_blank" class="source-item" style="border-left:2px solid #d45f8a22;"><span class="source-emoji">🧘</span><div><div class="source-name">Insight Timer</div><div class="source-tag" style="color:#d45f8a;">SOUL · PRACTICE</div></div></a>
      <a href="https://www.aljazeera.com" target="_blank" class="source-item" style="border-left:2px solid #4a9fd422;"><span class="source-emoji">🌍</span><div><div class="source-name">Al Jazeera</div><div class="source-tag" style="color:#4a9fd4;">WORLD · NEWS</div></div></a>
      <a href="https://restofworld.org" target="_blank" class="source-item" style="border-left:2px solid #4a9fd422;"><span class="source-emoji">🔌</span><div><div class="source-name">Rest of World</div><div class="source-tag" style="color:#4a9fd4;">WORLD · TECH</div></div></a>
      <a href="https://longreads.com" target="_blank" class="source-item" style="border-left:2px solid #4a9fd422;"><span class="source-emoji">📖</span><div><div class="source-name">Longreads</div><div class="source-tag" style="color:#4a9fd4;">WORLD · CULTURE</div></div></a>
      <a href="https://www.foreignaffairs.com" target="_blank" class="source-item" style="border-left:2px solid #4a9fd422;"><span class="source-emoji">🗺️</span><div><div class="source-name">Foreign Affairs</div><div class="source-tag" style="color:#4a9fd4;">WORLD · GEOPOLITICS</div></div></a>
      <a href="https://apod.nasa.gov" target="_blank" class="source-item" style="border-left:2px solid #4888c822;"><span class="source-emoji">🌌</span><div><div class="source-name">NASA APOD</div><div class="source-tag" style="color:#4888c8;">COSMOS · DAILY</div></div></a>
      <a href="https://www.pewresearch.org/religion/" target="_blank" class="source-item" style="border-left:2px solid #4888c822;"><span class="source-emoji">✦</span><div><div class="source-name">Pew: Religion</div><div class="source-tag" style="color:#4888c8;">COSMOS · DATA</div></div></a>
    </div>
...
<footer>
  <div class="inner" style="flex-direction:column;align-items:center;text-align:center;gap:20px;">
    <div style="font-size:26px;font-weight:700;letter-spacing:0.22em;font-family:'Courier New',monospace;"><span style="color:#c9a84c;">Us</span><span style="color:#8aa0b8;">Now</span></div>
    <div style="display:flex;gap:32px;flex-wrap:wrap;justify-content:center;align-items:baseline;">
      <span style="font-size:11px;letter-spacing:0.18em;color:#6a7a8a;font-family:'Courier New',monospace;">PERSPECTIVE</span>
      <span style="font-size:7px;color:#3a4a5a;font-family:'Courier New',monospace;">◆</span>
      <span style="font-size:11px;letter-spacing:0.18em;color:#6a7a8a;font-family:'Courier New',monospace;">FOCUS</span>
      <span style="font-size:7px;color:#3a4a5a;font-family:'Courier New',monospace;">◆</span>
      <span style="font-size:11px;letter-spacing:0.18em;color:#6a7a8a;font-family:'Courier New',monospace;">SCALE</span>
      <span style="font-size:7px;color:#3a4a5a;font-family:'Courier New',monospace;">◆</span>
      <span style="font-size:11px;letter-spacing:0.18em;color:#6a7a8a;font-family:'Courier New',monospace;">TRUST</span>
    </div>
    <div style="font-size:9px;letter-spacing:0.2em;color:#4a5a6a;font-family:'Courier New',monospace;">NON-PARTISAN &nbsp;·&nbsp; NON-DENOMINATIONAL &nbsp;·&nbsp; NON-NEGOTIABLE</div>
    <div style="width:40px;height:1px;background:rgba(122,96,48,0.2);"></div>
    <div style="font-size:9px;color:#4a5a6a;font-family:'Courier New',monospace;letter-spacing:0.1em;">LAST UPDATED · APRIL 15, 2026</div>
    <div style="font-size:9px;color:#4a5a6a;font-family:'Courier New',monospace;letter-spacing:0.1em;line-height:2;">All Rights Reserved for Copyrightable Content &nbsp;©&nbsp; 2026 &nbsp;USNow.app</div>
  </div>
</footer>
</div>
...
function buildFoundationMoons(){
  ...
  FOUNDATIONS.forEach(f=>{
    ...
    let _fTmoved=false,_fTsx=0,_fTsy=0;
    el.addEventListener('touchstart',e=>{
      _fTsx=e.touches[0].clientX;_fTsy=e.touches[0].clientY;_fTmoved=false;
      el.style.transform=`translateX(-50%) translateY(-8px) scale(1.09)`;
      el.style.boxShadow=`0 0 ${f.size*0.7}px ${f.glow},0 0 ${f.size*0.35}px ${f.border}88`;
    },{passive:true});
    el.addEventListener('touchmove',e=>{
      if(Math.abs(e.touches[0].clientX-_fTsx)>8||Math.abs(e.touches[0].clientY-_fTsy)>8){
        _fTmoved=true;
        el.style.transform=`translateX(-50%) translateY(0) scale(1)`;
        el.style.boxShadow='';
      }
...
if(window.visualViewport){
  let _modalViewportResizeTimer=null;
  window.visualViewport.addEventListener('resize',()=>{
    if(!document.getElementById('modal-overlay').classList.contains('open')) return;
    clearTimeout(_modalViewportResizeTimer);
    _modalViewportResizeTimer=setTimeout(()=>{
      applyZoomedSafariModalMode();
    },120);
  });
}
...
function renderCard(data){
  ...
  box.style.border=`3px solid rgba(${r},${g},${b},0.82)`;
  box.style.boxShadow=`inset 0 0 0 1px rgba(255,255,255,0.04),inset 0 0 22px rgba(${r},${g},${b},0.065),0 0 0 1px rgba(${r},${g},${b},0.16),0 18px 46px rgba(0,0,0,0.5),0 0 16px rgba(${r},${g},${b},0.14)`;
